...
test('capabilities expose only configured safe selections and defaults', () => {
  const capabilities = buildCapabilities({ codexModels: 'codex-a,codex-b', codexDefaultModel: 'codex-b', codexReasoningLevels: 'low,high', codexDefaultReasoning: 'high', claudeModels: 'sonnet,opus', claudeDefaultModel: 'opus', claudeReasoningLevels: 'low,medium,high', claudeDefaultReasoning: 'low', defaultAgent: 'claude' });
  ...
  assert.deepEqual(capabilities.agents[1], { id: 'claude', models: ['sonnet', 'opus'], reasoningLevels: ['low', 'medium', 'high'], defaults: { model: 'opus', reasoningLevel: 'low' } });
});
...
test('model and CLI-shaped injection values are rejected instead of forwarded', () => {
  ...
  assert.throws(() => validateSelection(capabilities, { agent: 'claude', model: 'sonnet', reasoningLevel: 'high' }), /not supported by claude/);
});

test('selection uses server defaults and inherits a run only for the same agent', () => {
  const capabilities = buildCapabilities({ codexModels: 'codex-a,codex-b', codexDefaultModel: 'codex-a', codexDefaultReasoning: 'medium', claudeModels: 'sonnet,opus', claudeReasoningLevels: 'low,medium,high', claudeDefaultReasoning: 'low' });
  ...
  assert.deepEqual(validateSelection(capabilities, { agent: 'claude' }, { agent: 'codex', model: 'codex-b', reasoningLevel: 'high' }), { agent: 'claude', model: 'sonnet', reasoningLevel: 'low' });
});
